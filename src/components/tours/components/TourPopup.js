...
class TourPopup extends Component {
	...
	saveTour(tour, building) {
		...
		if (this.validateForm()) {
			...
			const body = `GROUP TOUR REQUEST: My group would like to join the tour for ${building.building_alias} that is happening on ${moment(tour.selected_date).format('MMMM Do YYYY, HH:mm a')}. That tour was originally booked by ${tour.first_name}. My group is different and has ${this.state.group_size} people.`
			insertScheduledTour({
