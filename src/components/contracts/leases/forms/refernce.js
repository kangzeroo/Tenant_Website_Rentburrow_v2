...
class BasicLeaseForm extends Component {
	...
	constructor() {
      ...
      tenant_email: '',
			...
			submitted: false,
			error_messages: [],
			current_active_field: 'tenant_first_name',
			activeIndex: 1,
	...
	componentWillMount() {
		this.setState({
			tenant_first_name: this.props.tenant_profile.first_name ? this.props.tenant_profile.first_name : '',
			tenant_last_name: this.props.tenant_profile.last_name ? this.props.tenant_profile.last_name : '',
			tenant_phone_number: this.props.tenant_profile.phone ? this.props.tenant_profile.phone : '',
			tenant_email: this.props.tenant_profile.email ? this.props.tenant_profile.email : '',
		})
	}
  ...
  uploadPhoto(acceptedFiles, rejectedFiles, attr) {
	...
	formValidation() {
		...
		const errors = []
		if (this.state.tenant_first_name.length === 0 || this.state.tenant_last_name === 0 || this.state.tenant_phone_number.length === 0 || !this.state.tenant_email.length === 0) {
			errors.push('You must include your name, phone number and email')
		}
		if (!validateEmail(this.state.tenant_email) || !validateEmail(this.state.subletee_witness_email)) {
	...
	checkIfStepActive(step_number) {
		let active = false
		if (step_number === '1') {
			if (this.state.current_active_field === 'tenant_first_name' || this.state.current_active_field === 'tenant_last_name' || this.state.current_active_field === 'tenant_phone_number' || this.state.current_active_field === 'tenant_email' || this.state.current_active_field === 'subletee_student_card') {
				active = true
			}
	...
	checkIfStepComplete(step_number) {
		let complete = false
		if (step_number === '1') {
			if (this.state.tenant_first_name.length > 0 && this.state.tenant_last_name.length > 0 && this.state.tenant_phone_number.length > 0 && this.state.tenant_email.length > 0 && this.state.subletee_student_card && this.state.subletee_student_card.name && this.state.subletee_student_card.name.length > 0) {
				complete = true
			}
		} else if (step_number === '2') {
										...
										<div style={comStyles().student_form}>
											<Form.Field>
												...
												<input
													...
													onChange={(e) => this.updateAttr(e, 'tenant_first_name')}
											...
											<Form.Field>
												...
												<input
													...
													onChange={(e) => this.updateAttr(e, 'tenant_last_name')}
											...
											<Form.Field>
												...
												<input
													...
													onChange={(e) => this.updateAttr(e, 'tenant_phone_number')}
													value={this.state.tenant_phone_number}
											...
											<Form.Field>
												...
												<input
													placeholder='Email'
													onChange={(e) => this.updateAttr(e, 'tenant_email')}
...
BasicLeaseForm.propTypes = {
	history: PropTypes.object.isRequired,
	tenant_profile: PropTypes.object,
}

// for all optional props, define a default value
BasicLeaseForm.defaultProps = {
	 tenant_profile: {},
}

// Wrap the prop in Radium to allow JS styling
const RadiumHOC = Radium(BasicLeaseForm)
