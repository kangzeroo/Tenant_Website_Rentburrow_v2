// This file sets our API URLS used throughout the app
// toggle these to access development versus production servers

// LOCAL
// export const MESSAGING_MICROSERVICE = 'https://localhost:3001'
// export const FB_APP_ID = '115765432474914'
//
// export const SEARCH_MICROSERVICE = 'http://localhost:3002'
// export const CORP_MICROSERVICE = 'https://localhost:3001'
// export const REGISTER_MICROSERVICE = 'https://localhost:3001'
// export const GENERIC_MICROSERVICE = 'https://localhost:3001'
// export const FB_PARSER_MICROSERVICE = 'https://localhost:3001'

// PROD
export const MESSAGING_MICROSERVICE = 'https://34.195.86.132:3001'
export const FB_APP_ID = '1492022030811505'

export const SEARCH_MICROSERVICE = 'https://34.195.86.132:3002'
export const CORP_MICROSERVICE = 'https://34.195.86.132:3001'
export const REGISTER_MICROSERVICE = 'https://34.195.86.132:3001'
export const GENERIC_MICROSERVICE = 'https://34.195.86.132:3001'
export const FB_PARSER_MICROSERVICE = 'https://34.195.86.132:3001'
